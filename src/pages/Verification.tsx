...
export function Verification() {
  ...
  return (
    <div className="flex-1 w-full h-[calc(100vh-100px)] flex flex-col -m-6 -mb-8">
      ...
      `}</style>
      <div className="bg-white dark:bg-[#222736] border-b border-[#c3c5d7] dark:border-slate-700/60 px-6 py-3 flex items-center justify-between shrink-0 transition-colors">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/papers')} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full text-slate-600 dark:text-slate-300 transition-colors">
            <ArrowLeft className="w-5 h-5" />
          </button>
          <div>
            <h2 className="font-sans font-bold text-lg text-slate-900 dark:text-white leading-none">{paperName}</h2>
            <p className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest mt-1">Side-by-side Verification</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-1 bg-slate-50 dark:bg-[#1a1e29] border border-slate-200 dark:border-slate-700 rounded-lg p-1">
            <button onClick={() => setZoomLevel(Math.max(50, zoomLevel - 10))} className="p-1.5 hover:bg-white dark:hover:bg-[#252b3b] hover:shadow-sm rounded text-slate-600 dark:text-slate-300">
              <ZoomOut className="w-4 h-4" />
            </button>
            <span className="text-xs font-semibold w-12 text-center text-slate-700 dark:text-slate-200">{zoomLevel}%</span>
            <button onClick={() => setZoomLevel(Math.min(200, zoomLevel + 10))} className="p-1.5 hover:bg-white dark:hover:bg-[#252b3b] hover:shadow-sm rounded text-slate-600 dark:text-slate-300">
              <ZoomIn className="w-4 h-4" />
            </button>
          </div>
          <div className="h-6 w-px bg-slate-200 dark:bg-slate-700"></div>
          <button className="px-4 py-2 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 font-semibold text-xs rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800">Pause Review</button>
        </div>
      </div>

      <div className="flex-1 w-full flex border-t border-[#c3c5d7] dark:border-slate-700/60 overflow-hidden">
        <div className="w-1/2 h-full bg-[#525659] overflow-auto relative border-r border-[#c3c5d7] dark:border-slate-700/60">
        ...
        <div className="w-1/2 h-full bg-[#faf8ff] dark:bg-[#1a1e29] flex flex-col overflow-hidden relative transition-colors">

          <div className="bg-white dark:bg-[#222736] border-b border-[#c3c5d7] dark:border-slate-700/60 px-4 py-2.5 flex justify-between items-center shrink-0 transition-colors">
            <span className="text-[10px] font-bold text-[#003fb1] dark:text-blue-400 uppercase tracking-widest flex items-center gap-2">
          ...
          <div className="flex-1 overflow-y-auto min-h-0 p-6">
            <div className="max-w-xl mx-auto space-y-4">
              ...
              {loading ? (
                <div className="text-center p-8 text-slate-500 dark:text-slate-400">Loading question data...</div>
              ) : !question ? (
                <div className="text-center p-8 text-slate-500 dark:text-slate-400">No questions found.</div>
              ) : (
                <>
                  <div className="bg-white dark:bg-[#252b3b] border border-[#c3c5d7] dark:border-slate-700/70 rounded-xl p-4 shadow-sm transition-colors">
                    <label className="text-sm font-bold text-slate-800 dark:text-slate-100 mb-3 block">Extracted Question</label>
                    {previewMode ? (
                  ...
                  {question.question_type !== 'numerical' && question.options && (
                    <div className="bg-white dark:bg-[#252b3b] border border-[#c3c5d7] dark:border-slate-700/70 rounded-xl p-4 shadow-sm transition-colors">
                      <label className="text-sm font-bold text-slate-800 dark:text-slate-100 mb-3 block">Extracted Options</label>
                      <div className="space-y-3">
                        {Object.entries(question.options).map(([k, v]: [string, any], idx: number) => (
                          <div key={idx} className="flex gap-3 group">
                            <div className="w-9 h-9 mt-1 flex shrink-0 items-center justify-center rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-300 font-bold text-sm shadow-sm border border-slate-200 dark:border-slate-600 group-focus-within:bg-[#003fb1] dark:group-focus-within:bg-blue-600 group-focus-within:text-white transition-colors">{k}</div>
                  ...
                  <div className="bg-white dark:bg-[#252b3b] border border-[#c3c5d7] dark:border-slate-700/70 rounded-xl p-4 shadow-sm transition-colors">
                    <label className="text-sm font-bold text-slate-800 dark:text-slate-100 mb-3 block">Answer</label>
                    {previewMode ? (
                  ...
                  {question.explanation && (
                    <div className="bg-white dark:bg-[#252b3b] border border-[#c3c5d7] dark:border-slate-700/70 rounded-xl p-4 shadow-sm transition-colors">
                      <label className="text-sm font-bold text-slate-800 dark:text-slate-100 mb-3 block">Explanation</label>
                      {previewMode ? (
                  ...
                  <div className="bg-white dark:bg-[#252b3b] border border-[#c3c5d7] dark:border-slate-700/70 rounded-xl p-4 shadow-sm transition-colors">
                    <div className="flex items-center justify-between mb-3">
                      <label className="text-sm font-bold text-slate-800 dark:text-slate-100">Diagrams</label>
                  ...
                  <div className="bg-white dark:bg-[#252b3b] border border-[#c3c5d7] dark:border-slate-700/70 rounded-xl p-4 shadow-sm grid grid-cols-2 gap-4 transition-colors">
                    <div>
                      ...
                      <select
                        ...
                        className={`w-full text-sm border-slate-200 dark:border-slate-700 rounded p-1.5 focus:border-[#003fb1] dark:focus:border-blue-500 outline-none bg-slate-50 dark:bg-[#1a1e29] ${!question.subject ? 'text-slate-400' : 'text-slate-800 dark:text-slate-100'}`}
                      >
                    ...
                    <div>
                      <label className="text-[10px] font-bold text-slate-400 dark:text-slate-400 uppercase tracking-wider block mb-1">Tag / Topic</label>
                      <input
                        type="text"
                        value={question.tag || ''}
                        onChange={(e) => updateField('tag', e.target.value)}
                        className="w-full text-sm border-slate-200 dark:border-slate-700 rounded p-1.5 focus:border-[#003fb1] dark:focus:border-blue-500 outline-none bg-slate-50 dark:bg-[#1a1e29] text-slate-800 dark:text-slate-100 border"
                      />
                    </div>
                  </div>
                </>
          ...
          <div className="bg-white dark:bg-[#222736] border-t border-[#c3c5d7] dark:border-slate-700/60 px-6 py-5 flex justify-between items-center shrink-0 transition-colors">
            <button className="px-4 py-2 flex items-center gap-2 text-rose-600 dark:text-rose-400 font-semibold text-xs hover:bg-rose-50 dark:hover:bg-rose-950/40 rounded-lg transition-colors">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="px-8 py-2.5 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 disabled:opacity-50 text-slate-800 dark:text-slate-100 font-bold text-sm rounded-lg shadow-sm transition-colors flex items-center gap-2"
              >
                Previous
              </button>
              <button
