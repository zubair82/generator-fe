...
export const splitLatexLines = (str: string) => {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (str.startsWith('\\begin{', i)) {
      ...
      depth--;
    } else if (str.startsWith('\\left', i) && (i + 5 >= str.length || !/[a-zA-Z]/.test(str[i + 5]))) {
      leftRightDepth++;
    } else if (str.startsWith('\\right', i) && (i + 6 >= str.length || !/[a-zA-Z]/.test(str[i + 6]))) {
      leftRightDepth--;
    }
    ...
    // Handle stray \n which breaks KaTeX
    if (str.startsWith('\\n', i)) {
      const nextWordMatch = str.substring(i).match(/^\\n[a-zA-Z]+/);
      const validCmds = ['\\nabla', '\\neg', '\\neq', '\\ni', '\\not', '\\notin', '\\nu', '\\newline', '\\nexists', '\\nRightarrow', '\\nrightarrow', '\\nLeftrightarrow', '\\nleftrightarrow', '\\nleq', '\\ngeq', '\\nless', '\\ngtr', '\\nmid', '\\nparallel', '\\nvdash', '\\nvDash', '\\nVDash', '\\nVdash', '\\ntriangleleft', '\\ntriangleright', '\\ntrianglelefteq', '\\ntrianglerighteq', '\\nsubseteq', '\\nsupseteq', '\\nsubset', '\\nsupset', '\\ncong', '\\nsim', '\\napprox', '\\nsimeq', '\\nprec', '\\nsucc', '\\npreceq', '\\nsucceq'];
      if (!nextWordMatch || !validCmds.includes(nextWordMatch[0])) {
        if (depth === 0 && braceDepth === 0 && leftRightDepth === 0) {
          parts.push(currentPart.trim());
          currentPart = '';
        } else {
          currentPart += ' '; // inside environment, replace with space
        }
        i += 1; // skip the 'n'
        continue;
      }
...
export const processPlainText = (text: string) => {
  ...
  part = part.replace(/(?<!\\)(\\[a-zA-Z]+(?:_[a-zA-Z0-9]+|\^[a-zA-Z0-9]+)?(?:\[[^\]]*\])?(?:\s*\{[^}{]*(?:\{[^}{]*(?:\{[^}{]*(?:\{[^}{]*\}[^}{]*)*\}[^}{]*)*\}[^}{]*)*\})+)/g, '\uE000$1\uE001');
  ...
  // Strip nested \uE000 and \uE001 markers BEFORE the exponent regex to prevent it from erroneously splitting nested blocks
  let unnested = '';
  let innerDepth = 0;
  for (let i = 0; i < part.length; i++) {
    if (part[i] === '\uE000') {
      if (innerDepth === 0) unnested += '\uE000';
      innerDepth++;
    } else if (part[i] === '\uE001') {
      innerDepth--;
      if (innerDepth === 0) unnested += '\uE001';
...
export const smartFormatMath = (md: any) => {
  ...
  // Fix LLM artifacts like {$^3$}$C_1 -> ${}^3 C_1$
  // Consume optional trailing $ to prevent generating $$ which breaks inline math parsing.
  p = p.replace(/\{\$([\^_][^$]*)\$\}(\s*)\$?/g, '${}$1$2');

  // Fix LLM artifacts like \left { and \right } which should be \left \{ and \right \}
  p = p.replace(/\\left\s*\{/g, '\\left\\{').replace(/\\right\s*\}/g, '\\right\\}');

  // Fix literal OCR newline markers (e.g. \\\n or \\n or \n) BEFORE doing anything else!
  // We use just \\n instead of \\+n so we don't accidentally destroy preceding LaTeX backslashes (like \\\\ for row breaks)
  p = p.replace(/\\n(?![a-zA-Z])/g, '\n');
    ...
    if (i % 2 === 0) {
      ...
      while (j < subParts.length) {
        ...
        if (j < subParts.length) {
          ...
          if (block.startsWith('\\begin') || block.startsWith('\\left')) {
            processedText += '\uE008' + block + '\uE009';
          } else {
            processedText += block;
          }
  ...
  // Convert environments that strictly require display mode (like \begin{align*}) to $$
  p = p.replace(/\uE008/g, '$$$$').replace(/\uE009/g, '$$$$');
